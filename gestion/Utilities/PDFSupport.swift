...
// MARK: - PDF Renderer avec pagination correcte
...
enum PDFRenderer {
    ...
    static func renderQuotePDF(for quote: Quote) -> Data {
        ...
        startPage(context, mediaBox: &mediaBox)
        var y: CGFloat = pageHeight - 48

        y = drawHeader(for: quote, context: context, y: y)
        y = drawClientBlock(for: quote, context: context, y: y)
        y = drawSummaryBlock(for: quote, context: context, y: y)
        y = drawLinesTable(for: quote, context: context, startY: y, mediaBox: &mediaBox)
        ...
        // ✅ restoreGraphicsState AVANT endPDFPage — corrige la corruption PDF
        endPage(context)
        context.closePDF()
        return data as Data
    }
    ...
    private static func startPage(_ context: CGContext, mediaBox: inout CGRect) {
    ...
    private static func endPage(_ context: CGContext) {
        NSGraphicsContext.restoreGraphicsState()  // ✅ toujours appelé, jamais oublié
        context.endPDFPage()
    }
    ...
    @discardableResult
    private static func drawHeader(for quote: Quote, context: CGContext, y: CGFloat) -> CGFloat {
        ...
        draw("123 avenue de l'Énergie, 30000 Nîmes",            at: CGPoint(x: marginX, y: y - 26), attrs: body)
        draw("contact@electricite-perez.fr  •  06 00 00 00 00", at: CGPoint(x: marginX, y: y - 40), attrs: body)
    ...
    @discardableResult
    private static func drawClientBlock(for quote: Quote, context: CGContext, y: CGFloat) -> CGFloat {
        ...
        NSBezierPath(roundedRect: CGRect(x: marginX, y: y - 60, width: 511, height: 54), xRadius: 10, yRadius: 10).fill()

        draw("Facturé à",                                            at: CGPoint(x: 56, y: y - 16), attrs: heading)
        draw(quote.clientName,                                       at: CGPoint(x: 56, y: y - 32), attrs: body)
    ...
    @discardableResult
    private static func drawSummaryBlock(for quote: Quote, context: CGContext, y: CGFloat) -> CGFloat {
        guard !quote.summary.isEmpty else { return y - 8 }
        let body: Attrs = [.font: NSFont.systemFont(ofSize: 11), .foregroundColor: NSColor.black]
        draw("Description : \(quote.summary)",
             in: CGRect(x: marginX, y: y - 42, width: 511, height: 38),
             attrs: body)
        return y - 50
    }
    ...
    private static func drawLinesTable(
        ...
        let hdr:   Attrs = [.font: NSFont.boldSystemFont(ofSize: 10), .foregroundColor: NSColor.black]
        ...
        // Dessine l'en-tête du tableau, retourne le Y après l'en-tête
        func tableHeader(at y: CGFloat, suite: Bool = false) -> CGFloat {
            NSColor(calibratedWhite: 0.92, alpha: 1).setFill()
            NSBezierPath(rect: CGRect(x: marginX, y: y - 22, width: 511, height: 22)).fill()
            draw(suite ? "Prestation (suite)" : "Prestation", at: CGPoint(x: 48,  y: y - 16), attrs: hdr)
            draw("Qté",       at: CGPoint(x: 292, y: y - 16), attrs: hdr)
            draw("PU HT",     at: CGPoint(x: 338, y: y - 16), attrs: hdr)
            draw("TVA",       at: CGPoint(x: 412, y: y - 16), attrs: hdr)
            draw("Total TTC", at: CGPoint(x: 462, y: y - 16), attrs: hdr)
            return y - 28
        }

        var y = tableHeader(at: startY)

        for line in quote.lines {
            if y - rowH < marginBottom + 180 {
                y = newPage(context, mediaBox: &mediaBox)
                y = tableHeader(at: y, suite: true)
            }
            ...
            sep.move(to: CGPoint(x: marginX, y: y - rowH + 2))
            sep.line(to: CGPoint(x: 553,    y: y - rowH + 2))
            sep.lineWidth = 0.5
            sep.stroke()
    ...
    private static func drawTotals(for quote: Quote, context: CGContext, bottomY: CGFloat) {
        let hdr:    Attrs = [.font: NSFont.boldSystemFont(ofSize: 11), .foregroundColor: NSColor.black]
        let body:   Attrs = [.font: NSFont.systemFont(ofSize: 11),     .foregroundColor: NSColor.black]
        ...
        for (label, value, bold) in rows {
            draw(label, at: CGPoint(x: 340, y: y), attrs: bold ? accent : hdr)
            draw(value, at: CGPoint(x: 470, y: y), attrs: bold ? accent : body)
            y -= 20
        }
    }

    private static func drawPayments(for quote: Quote, context: CGContext, bottomY: CGFloat) {
        let hdr:  Attrs = [.font: NSFont.boldSystemFont(ofSize: 11), .foregroundColor: NSColor.black]
        let body: Attrs = [.font: NSFont.systemFont(ofSize: 10),     .foregroundColor: NSColor.black]

        var y = bottomY + CGFloat(min(quote.payments.count, 4) * 18) + 18
        draw("Historique des paiements", at: CGPoint(x: marginX, y: y), attrs: hdr)
        y -= 18
    ...
    private static func drawFooter(context: CGContext) {
        ...
        draw("Merci pour votre confiance. Document généré depuis Gestion Électricien.",
             at: CGPoint(x: marginX, y: 24), attrs: attrs)
    }
