...
struct ContentView: View {
                                ...
                                NavigationLink {
                                    ...
                                    HStack(spacing: 14) {
                                        VStack(alignment: .leading, spacing: 4) {
                                            Text(intervention.timeSlot).font(.headline)
                                            Text(intervention.clientName).font(.subheadline.weight(.semibold))
                                            Text(intervention.kind).font(.footnote).foregroundStyle(.secondary)
                                        }
                                        Spacer()
                                        VStack(alignment: .trailing, spacing: 6) {
                                            StatusBadge(title: intervention.statusValue.label, color: intervention.statusValue.color)
                                            Text(intervention.location).font(.footnote).foregroundStyle(.secondary)
                                        }
                                    }
                                    .padding(14)
                                    ...
                                    VStack(alignment: .leading, spacing: 6) {
                                        HStack(alignment: .top) {
                                            VStack(alignment: .leading, spacing: 4) {
                                                ...
                                                Text(client.city).foregroundStyle(.secondary)
                                            }
                                            Spacer()
                                            StatusBadge(title: client.statusValue.label, color: client.statusValue.color)
                                        }
                                        Text(client.note).font(.footnote).foregroundStyle(.secondary)
                                        HStack {
                                ...
                                VStack(alignment: .leading, spacing: 10) {
                                    HStack {
                                        VStack(alignment: .leading, spacing: 2) {
                                            Text(intervention.clientName).font(.headline)
                                            Text(intervention.location).font(.subheadline).foregroundStyle(.secondary)
                                        }
                                        Spacer()
                                        StatusBadge(title: intervention.statusValue.label, color: intervention.statusValue.color)
                                    }
                                    ...
                                    HStack {
                                        Text(intervention.dateLabel).font(.footnote.weight(.semibold))
                                        Spacer()
                            ...
                            cardRow {
                                HStack(alignment: .top, spacing: 14) {
                                    ...
                                    VStack(alignment: .leading, spacing: 6) {
                                        HStack {
                                            VStack(alignment: .leading, spacing: 2) {
                                                Text(quote.reference).font(.headline)
                                                Text(quote.documentTypeValue.label).font(.caption).foregroundStyle(.secondary)
                                            }
                                            Spacer()
                                            Text(quote.formattedAmount).font(.headline)
                                        }
                                        Text(quote.clientName).foregroundStyle(.secondary)
                                        HStack {
                            ...
                            cardRow {
                                HStack(spacing: 14) {
                                    ...
                                    VStack(alignment: .leading, spacing: 4) {
                                        ...
                                        Text("Réf. \(item.sku)").font(.footnote).foregroundStyle(.secondary)
                                    ...
                                    VStack(alignment: .trailing, spacing: 4) {
                                        Text("\(item.movements.count) mouvements").font(.headline)
    ...
    private func scheduleRow(for intervention: InterventionRecord) -> some View {
        HStack(spacing: 14) {
            ...
            VStack(alignment: .trailing, spacing: 4) {
                StatusBadge(title: intervention.statusValue.label, color: intervention.statusValue.color)
                Text(intervention.usedItemsCost.formattedEuro).font(.caption).foregroundStyle(.secondary)
            }
        }
        .padding(14)
        .background(Color.white.opacity(0.82), in: RoundedRectangle(cornerRadius: 18, style: .continuous))
        ...
        HStack(alignment: .top, spacing: 12) {
            ...
            Menu(content: menu) {
                Image(systemName: "ellipsis.circle")
                    .font(.title3).foregroundStyle(.secondary).padding(.top, 2)
            }
        }
        .padding(14)
    ...
    private func editorSheetContent(for sheet: EditorSheet) -> some View {
        ...
        case .intervention(let draft):
            InterventionFormView(draft: draft, clients: clientOptions, inventoryOptions: inventoryOptions) {
